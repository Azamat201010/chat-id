...
var stat = "other";
var ism="";
const supApi = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBia216ZGZhc2t6aWZ2c3hyaXpkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTM4MDg4NjgsImV4cCI6MjA2OTM4NDg2OH0.N4HysjRwrACKFmZZF0q51_9K1dvWRpRjAUeFO5F3y1s";
...
async function sendMessage() {
    ...
    const{data,error}=await sardor.from('massages').insert([{'user':user.value.trim(),'msg':txt.value.trim(),time_insert:vaqt}]).select()
    txt.value="";
...
async function kirish() {
    ...
    const {data:odam,error:nito}=await sardor.from('users')
                                .select('*')
                                .eq('name',ism)
                                .eq('pass',pass)
